...
const drawFrame = (time) => {
    ...
    drawFilledCircle((time/5)%width, (time/10 + time)%height, 25, 'blue');
    drawFilledCircle((time/7)%width, (time/10 + time)%height, 25, 'black');
    drawFilledCircle((time/12)%width, (time/10 + time)%height, 25, 'red');
}
